...
public class TemplateDemo {
  ...
  public static void main(String[] args) {
    try (OpenShiftClient openshiftClient = new KubernetesClientBuilder().build().adapt(OpenShiftClient.class)) {
      ...
      openshiftClient.resourceList(resourceList).inNamespace(namespace).create();

      final CountDownLatch latch = new CountDownLatch(1);
      Watch watch = openshiftClient.pods().inNamespace(namespace).withLabel("name", "wordpress-mysql-example")
          .watch(new Watcher<>() {
